...
(if (and nil desktop-menu-mode-map)
  ...
  (let ((key ?1))
    (while (<= key ?9)
      (define-key desktop-menu-mode-map (char-to-string key)
'digit-argument)
      (setq key (1+ key))))
  ...
  (define-key desktop-menu-mode-map "c"
'desktop-menu-change-directory)
  (define-key desktop-menu-mode-map "x"       'desktop-menu-clear)
  (define-key desktop-menu-mode-map "^"
'desktop-menu-up-directory)
...
\\[desktop-menu-select] and save current Emacs desktop by
\\[desktop-menu-save].
Leave buffer by \\[desktop-menu-quit].  Abort buffer list by
\\[desktop-menu-abort].
...
\\[desktop-menu-merge]   -- Merge current lines desktop to current
desktop.
...
(defun desktop-menu-save-main-list (desktops directory)
  "Save desktops DESKTOPS in DIRECTORY in file
`desktop-menu-list-file'."
...
`desktop-menu-list-file' for desktop list."
  ...
  (if read-main-p
      (setq desktop-menu-desktops (desktop-menu-read-main-list
directory)))
  (desktop-menu-mode)
...
(defun desktop-menu-lines-desktop ()
  ...
  (let ((line (1- (count-lines (point-min) (if (eobp) (point) (1+
(point)))))))
    (if (and (>= line 1)
