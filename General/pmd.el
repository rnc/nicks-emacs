...
(defcustom pmd-ruleset-list "rulesets/basic.xml"
...
;; RNC MODIFIED TO REMOVE LIB
;;         (pmd-lib     (concat pmd-home path-slash "lib" path-slash)))
         (pmd-lib     (concat pmd-home path-slash path-slash)))
    (concat "\'"
            (mapconcat
             (lambda (path)
               path)
...
(defun pmd-file-or-dir (target)
  ...
  (let ((pmd-command
         (concat pmd-java-home " -classpath " (pmd-classpath) " net.sourceforge.pmd.PMD "
                 target " emacs " (concat pmd-ruleset-list))))
